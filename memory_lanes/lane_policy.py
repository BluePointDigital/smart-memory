...
class LanePolicy:
    ...
    def should_promote_to_core(self, memory: BaseMemory) -> LaneDecision:
        ...
        return LaneDecision(True, LaneName.CORE, "durable high-confidence memory", priority=max(memory.pinned_priority, memory.importance_score))
    ...
    def should_promote_to_working(self, memory: BaseMemory, *, now: datetime | None = None) -> LaneDecision:
        ...
        if memory.source_session_id in {"", "unknown", "placeholder", "legacy_unknown"}:
        ...
        if is_task_like and (is_recent or has_task_tag or session_relevant):
            return LaneDecision(True, LaneName.WORKING, "active working-context memory", priority=min(1.0, max(memory.importance_score, memory.confidence)))
